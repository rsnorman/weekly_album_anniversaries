...
namespace :weekly_albums do
  ...
  task set_twitter_screen_name: :environment do
    ...
    Rails.logger.info 'Setting Twitter screen names...'
    Artist
      ...
      .limit(1)
      .each do |artist|
        Rails.logger.info "Finding twitter screen name for #{artist.name}…"
        if WistfulIndie::Twitter::ScreenNameAssigner.new(artist).assign
          Rails.logger.info "Found Twitter screen name: @#{artist.twitter_screen_name}"
        else
          Rails.logger.info "Could not find twitter screen name for #{artist.name}"
        end
      end
    Rails.logger.info 'Finished setting all artist screen names'
  end
end
